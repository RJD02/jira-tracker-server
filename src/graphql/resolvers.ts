...
export const resolvers = {
  Query: {
    ...
    issues: async function (
      _: any,
      { project, forced = false }: { project: string; forced?: Boolean }
    ) {
    ...
    jiraprojects: async function (
      _: any,
      {
        baseurl,
        token,
        username,
      }: { baseurl: string; token: string; username: string }
    ) {
      try {
        ...
        const assigneesData = await Promise.all(
          jiraData.map(async (member) => {
            ...
            if (!project) {
              ...
              throw new Error(
                `Project not registered for projectId: ${member.projectId}`
              );
            }
